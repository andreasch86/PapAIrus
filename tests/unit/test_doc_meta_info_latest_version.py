...
def build_tree():
    """
    Build a tree structure for documentation items.
    
    Returns:
        A tuple containing the root, directory, file item, and function objects.
    """
...
def test_need_to_generate_respects_status_and_ignore():
    """
    Test the `need_to_generate()` function to ensure it respects the status and ignore list.
...
def test_tree_helpers_compute_depth_and_paths():
    """
    Test the tree helper functions for computing depth and paths.
...
def test_find_all_referencer_clamps_column_on_blank_line(tmp_path):
    """
    Test the `find_all_referencer()` function to ensure it handles blank lines correctly.
    
    Args:
        tmp_path: A temporary path to use for the test.
