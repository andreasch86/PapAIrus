...
def test_run_warns_on_main_branch(monkeypatch):
    class FakeBranch:
        ...
        Attributes:
            name (str): Name of the branch. Defaults to "main".
        """
        name = "main"
    ...
    class FakeRepo:
        ...
        Attributes:
            active_branch (FakeBranch): Active branch of the repository. Defaults to a FakeBranch with name "main".
        
        Methods:
            is_dirty(self, untracked_files=True) -> bool: Returns True if the repository is dirty (has uncommitted changes).
        """
        active_branch = FakeBranch()

        def is_dirty(self, untracked_files=True):
            """
            Checks if the repository is dirty (has uncommitted changes).
            
            Args:
                self (FakeRepo): The repository to check.
                untracked_files (bool): Whether to include untracked files in the dirty check. Defaults to True.
...
def test_run_handles_detached_head(monkeypatch, tmp_path):
    """
    Test case for the `create_documentation` command when the repository is in a detached head state.
    
    Args:
        monkeypatch (MonkeyPatch): MonkeyPatch object to patch the `git.Repo` and `Runner` classes.
        tmp_path (Path): Path to the temporary directory where the documentation will be created.
...
def test_diff_reports_no_tasks(monkeypatch):
    """
    test_diff_reports_no_tasks function.
    """
    fake_tree = types.SimpleNamespace(
        has_task=False,
        print_recursive=lambda diff_status, ignore_list: None,
...
def test_module_entrypoint_import_does_not_trigger_cli():
    """
    Test that importing the `papairus.__main__` module does not trigger the `cli()` function.
    
    This test ensures that the `cli()` function is not accidentally executed when the module is imported.
    """
    import importlib
    ...
    def fake_cli():
        """
        Fake CLI function for testing.
        
        This function does not actually perform any CLI functionality. Instead, it records the fact that it was called in the `calls` dictionary.
        """
        calls["cli"] = True
...
def test_generate_docstrings_cli_dry_run(tmp_path):
    """
    Test that the `generate_docstrings` CLI command runs in dry-run mode.
    
    This test creates a sample Python file and runs the `generate_docstrings` command with the `--dry-run` option. It then verifies that the docstrings are not updated in the file.
    """
...
def test_generate_docstrings_cli_with_llm(monkeypatch, tmp_path):
    """
    Test that the `generate_docstrings` CLI command uses the LLM client.
    
    This test sets up a mock LLM client and runs the `generate_docstrings` command with the `--backend` option set to `gemini`. It then verifies that the LLM client is used to generate the docstrings.
    """
    created = {}
    captured_settings = {}
    ...
    class DummyGenerator:
        """
        Dummy generator class for testing.
        
        This class simulates the functionality of the `DocstringGenerator` class. It records the backend and LLM client used in the constructor and the dry-run flag used in the `run()` method.
        """
