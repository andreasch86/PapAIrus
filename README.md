...
- **Gemini API**: set `GEMINI_API_KEY` (for API-key models) or provide a service account JSON (`--gemini-credentials-file`/`GOOGLE_APPLICATION_CREDENTIALS`) for Gemini 3; optionally override `--base-url`.
...
# - API key for supported Gemini models
export GEMINI_API_KEY=your_gemini_key
# - or service account JSON (required for Gemini 3)
export GOOGLE_APPLICATION_CREDENTIALS=/path/to/service_account.json

papairus run --model gemini-3-flash --allow-main --telemetry \
  --gemini-credentials-file "$GOOGLE_APPLICATION_CREDENTIALS"
